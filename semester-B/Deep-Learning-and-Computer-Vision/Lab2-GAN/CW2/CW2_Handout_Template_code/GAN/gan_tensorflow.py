...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    outputs = sess.run(G_z, feed_dict={z: z_})
    outputs = outputs * 0.5 + 0.5
    canvas = np.empty((28*5, 28*5))
    for i in range(5):
        for j in range(5):
            canvas[i * 28:(i + 1) * 28, j * 28:(j + 1) * 28] = outputs[i * 5 + j].reshape([28, 28])
...
train_hist = {'D_losses': [], 'G_losses': [], 'per_epoch_ptimes': [], 'total_ptime': []}
# training-loop
