function [clustCent,N_members,sum_w] = MeanShiftCluster_modified(dataPts,bandWidth)
% function [clustCent,data2cluster,cluster2dataCell] = MeanShiftCluster_modified(dataPts,bandWidth)
...
%**** Initialize stuff ***
dim_dataPts = size(dataPts,1);
w = dataPts(dim_dataPts,:);
if(sum(w)==0)
    clustCent = [];
    return;
end
dataPts = dataPts(1:dim_dataPts-1,:);
...
%         myMean      = mean(dataPts(:,inInds),2);                %compute the new mean
        if(sum(w(:,inInds))<0.00000000000001)
            return;
        end
        myMean = sum((dataPts(:,inInds).*repmat(w(:,inInds),numDim,1)),2)/sum(w(:,inInds)+eps);
...
sum_w = zeros(1,numClust);
N_members = zeros(1,numClust);
if nargout > 2
%     cluster2dataCell = cell(numClust,1);
    for cN = 1:numClust
        myMembers = data2cluster == cN;
        sum_w(cN) = sum(w(:,myMembers));
        N_members(1,cN) = sum(myMembers);
